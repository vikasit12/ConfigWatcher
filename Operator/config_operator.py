...
# It takes the configmap name as the argument and uses it to search for configwatcher
# that have the configmap name in its spec
def getPodLabels(configmap):
    url = "{}/apis/k8spatterns.io/v1/namespaces/{}/configwatchers".format(
        base_url, namespace)
    r = requests.get(url)
    ...
    pod_labels_json = [i['spec']['podSelector']
                       for i in response['items'] if i['spec']['configMap'] == configmap]
